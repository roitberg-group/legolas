...
class StandardRadial(torch.nn.Module):
    ...
    @classmethod
    def cover_linearly(cls, eta: float, num_shifts: int, start: float = 0.9, cutoff: float = 10.4, cutoff_fn='cosine'): #keeping start the same
    ...
    def like_1x(cls, **kwargs):
        ...
        m = cls.cover_linearly(cutoff=10.4, eta=16.0, num_shifts=32, **kwargs)
        if exact:
    ...
    def like_2x(cls, **kwargs):
        exact = kwargs.pop('exact', True)
        m = cls.cover_linearly(cutoff=10.2, eta=19.7, num_shifts=32, start=0.8, **kwargs) #same eta
        ...
        m.ShfR[0, 7] = 2.681250095                                                                 ###change this?
        if exact:
...
class StandardAngular(torch.nn.Module):
    ...
    def cover_linearly(cls, eta: float, num_shifts: int, zeta: float,
            num_angle_sections: int, start: float = 0.9, cutoff: float = 10.4, cutoff_fn='cosine'):
    ...
    def like_1x(cls, **kwargs):
        exact = kwargs.pop('exact', True)
        m = cls.cover_linearly(cutoff=7.0, eta=8.0, zeta=32.0, num_shifts=8, num_angle_sections=16, **kwargs) #eta, zeta  unchanged
        if exact:
    ...
    def like_2x(cls, **kwargs):
        exact = kwargs.pop('exact', True)
        m = cls.cover_linearly(cutoff=7.0, eta=12.5, num_shifts=16, start=0.8, zeta=14.1, num_angle_sections=8, **kwargs) #this is flipped from like_1x?
        if exact:
